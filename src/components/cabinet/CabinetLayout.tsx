...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: "Диалог", icon: "MessageSquare", to: "/cabinet/chat" },
  { label: "Документы", icon: "FileText", to: "/cabinet/documents" },
  { label: "Команда", icon: "Users", to: "/cabinet/team" },
  { label: "Настройки", icon: "Settings", to: "/cabinet/settings" },
];
    ...
    <div className="flex h-screen overflow-hidden bg-[#0a0a0f]">
      ...
      >
        ...
        <div className="flex h-16 items-center gap-3 border-b border-[#1a1a2e] px-5">
          ...
          <div className="min-w-0">
            <p className="truncate font-oswald text-sm font-medium uppercase tracking-wider text-white/90">
              {company?.name ?? "Кабинет"}
            </p>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-16 shrink-0 items-center gap-4 border-b border-[#1a1a2e] bg-[#0a0a12]/80 px-4 backdrop-blur-sm lg:px-6">
          ...
          {/* KM Balance */}
          <div className="flex items-center gap-2 rounded-md border border-[#1a1a2e] bg-[#0f0f18] px-3 py-1.5">
            <Icon name="Wallet" size={14} className="text-cyan-400" />
            <span className="font-mono-ibm text-xs text-white/70">
              {formatKM(0)}
            </span>
          </div>
